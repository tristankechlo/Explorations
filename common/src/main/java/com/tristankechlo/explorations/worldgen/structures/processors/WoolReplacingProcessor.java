...
public class WoolReplacingProcessor extends StructureProcessor {
    ...
    private BlockState getNewWool(String color) {
        ...
        if (Registry.BLOCK.containsKey(location)) {
            return Registry.BLOCK.get(location).defaultBlockState();
        }
        return null;
    }
    ...
    private boolean isWoolBlock(BlockState state) {
        String block_id = Registry.BLOCK.getKey(state.getBlock()).toString();
